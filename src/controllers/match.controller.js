...
function addMatchAdmin(req, res) {
    ...
    if ( params.idTeam1 && params.idTeam2 && params.goal1 && params.goal2) {
        if(params.idTeam1 == params.idTeam2) return res.status(500).send({ menssage:'No se pueden poner dos equipos iguales'})
        if(params.goal1 < 0 || params.goal2 < 0) return res.status(500).send({ menssage:'No se pueden ingresar valores negativos en los goles'})
        League.find({ _id: idLeague }).exec((err, leagueFound) => {
            if (err) return res.status(500).send({ menssage: 'Error en la petición buscar liga' })
            if (leagueFound.length <= 0) return res.status(404).send({ menssage: 'No se encontro la liga' })

            if (leagueFound) {
                Team.find({ _id: params.idTeam1 }).exec((err, team1Found) => {
                    if (err) return res.status(500).send({ menssage: 'Error en la petición buscar Team1' })
                    if (team1Found.length <= 0) return res.status(404).send({ menssage: 'No se encontro el Team 1' })
                    if (team1Found) {
                        Team.find({ _id: params.idTeam2 }).exec((err, team2Found) => {
                            if (err) return res.status(500).send({ menssage: 'Error en la petición buscar Team2' })
                            if (team2Found.length <= 0) return res.status(404).send({ menssage: 'No se encontro el Team 2' })
                            if (team2Found) {
                                ...
                                matchModel.save((err, matchSaved) => {
                                    if (err) return res.status(500).send({ menssage: 'Error al guardar match' })
                                    if (matchSaved) {
                                        if (params.goal1 > params.goal2) {
                                            Team.findByIdAndUpdate(params.idTeam1, { $inc: { pts: 3, pj: 1, pg: 1, goalA: params.goal1, goalC: params.goal2} }, (err, team1Updated) => {
                                                if (err) return res.status(500).send({ menssage: "error en la peticion actualizar pts team 1 win1" })
                                                if (team1Updated) {
                                                    Team.findByIdAndUpdate(params.idTeam2, { $inc: { pj: 1, pp: 1,goalA: params.goal2, goalC: params.goal1} }, (err, team2Updated) => {
                                                        if (err) return res.status(500).send({ menssage: "error en la peticion actualizar pj team2" })
                                                        if (!team2Updated) return res.status(500).send({ menssage: "error guardar update pj team 2" })
                                                    })
                                                }
                                        ...
                                        if (params.goal1 < params.goal2) {
                                            Team.findByIdAndUpdate(params.idTeam2, { $inc: { pts: 3, pj: 1, pg: 1, goalA: params.goal2, goalC: params.goal1} }, (err, team2Updated) => {
                                                if (err) return res.status(500).send({ menssage: "error en la peticion actualizar pts team 2 win2" })
                                                if (team2Updated) {
                                                    Team.findByIdAndUpdate(params.idTeam1, { $inc: { pj: 1, pp: 1,goalA: params.goal1, goalC: params.goal2} }, (err, team1Updated) => {
                                                        if (err) return res.status(500).send({ menssage: "error en la peticion actualizar pj team1" })
                                                        if (!team1Updated) return res.status(500).send({ menssage: "error guardar update pj team 1" })
                                                    })
                                                }
                                        ...
                                        if (params.goal1 == params.goal2) {
                                            Team.findByIdAndUpdate(params.idTeam1, { $inc: { pts: 1, pj: 1, pe: 1, goalA: params.goal1, goalC: params.goal2 } }, (err, team1Updated) => {
                                                if (err) return res.status(500).send({ menssage: "error en la peticion actualizar pts team 1 draw1" })
                                                if (team1Updated) {
                                                    Team.findByIdAndUpdate(params.idTeam2, { $inc: { pts: 1, pj: 1, pe: 1, goalA: params.goal2, goalC: params.goal1 } }, (err, team2Updated) => {
                                                        if (err) return res.status(500).send({ menssage: "error en la peticion actualizar pts team 2 draw2" })
                                                        if (!team2Updated) return res.status(500).send({ menssage: "error al guardar el team2 pts update draw2" })
                                                    ...
                                                    return res.status(500).send({ menssage: 'error en guardar draw1' })
                                                }
                                        ...
                                        return res.status(500).send({ menssage: 'No se guardo match' })
                                    }
                                })
                            } else {
                                return res.status(500).send({ menssage: 'No se encontro Team2' })
                            }
                        })

                    } else {
                        return res.status(500).send({ menssage: 'No se encontro Team1' })
                    }
                })
            } else {
                return res.status(500).send({ menssage: 'No se encontro liga con ese id' })
            }
        ...
        return res.status(500).send({ menssage: 'Ingrese todos los parametros que se solicitan' })
    }
} else {
...
function addMatchUser(req, res) {
    ...
    if ( params.idTeam1 && params.idTeam2 && params.goal1 && params.goal2) {
        ...
        League.find({ _id: idLeague }).exec((err, leagueFound) => {
            if (err) return res.status(500).send({ menssage: 'Error en la petición buscar liga' })
            if (leagueFound.length <= 0) return res.status(404).send({ menssage: 'No se encontro la liga' })
            if(leagueFound[0]['owner'] != req.user.sub) return res.status(500).send({ menssage:'no puede agregar partidos a otras ligas'})
            if (leagueFound) {
                Team.find({ _id: params.idTeam1 }).exec((err, team1Found) => {
                    if (err) return res.status(500).send({ menssage: 'Error en la petición buscar Team1' })
                    if (team1Found.length <= 0) return res.status(404).send({ menssage: 'No se encontro el Team 1' })
...
function deleteMatchUser(req, res){
    ...
    if(params.idMatch){
        Match.find({_id: params.idMatch}).exec((err, matchFound) => {
            if(err) return res.status(500).send({ menssage: 'Error en la petición buscar match'})
            if(matchFound && matchFound.length > 0){
                var idLeague  = matchFound[0]["league"]
                var idMatch1 = matchFound[0]["_id"]
                
                League.find({_id: idLeague}).exec((err, leagueFound) => {
                    if(err) return res.status(500).send({ menssage:"error al buscar la liga del match"})
                    if(leagueFound && leagueFound.length > 0){
                   let idLeagueUser = leagueFound[0]["owner"];
                    
                    if(idLeagueUser == req.user.sub){
                          Match.findByIdAndDelete(idMatch1,(err,matchDeleted) => {
                            if(err) return res.status(500).send({ menssage:"Error en la peticion eliminar match"})
                            ...
                            if(matchDeleted){
                                ...
                                return res.status(500).send({ menssage: 'No se encotro el match que desea elimianr 2'})
                            }


                          })
                    }else{
                        return res.status(500).send({ menssage: 'No puede eliminar match que no pertenezcan a su liga'})
                    }
                    
                    }else{
                        return res.status(500).send({ menssage: 'No existe una liga con es id del match'})
                    }
                })
            }else{
                return res.status(500).send({ menssage: 'No existe un match con ese id'})
            }
        ...
        return res.status(500).send({ menssage: 'Ingrese los parametros solcitados'})
    }
}
...
function listTablesAdmin(req,res) {
    ...
    if(req.user.role == 0){
        Team.find({idLeague: params.idLeague}).sort({pts: -1, goalA:-1}).exec((err,tableFound)=>{
            if(err) return res.status(500).send({ menssage:'Error en la peticion buscar tabla'})
            if(tableFound.length <= 0) return res.status(500).send({ menssage:'No existen equipos en esa liga o no existe esa liga'})
...
function listTableUser(req,res) {
    ...
    if(params.idLeague){
        League.find({ _id: params.idLeague}).exec((err,leagueFound)=>{
            ...
            if(leagueFound[0]["owner"] != req.user.sub) return res.status(500).send({menssage:'No puede ver una liga que no creo'})

            Team.find({idLeague: params.idLeague}).sort({pts: -1, goalA:-1}).exec((err,tableFound)=>{
                if(err) return res.status(500).send({ menssage:'Error en la peticion buscar tabla'})
                if(tableFound.length <= 0) return res.status(500).send({ menssage:'No existen equipos en esa liga'})
                return res.status(200).send({tableFound})
        ...
        return res.status(500).send({ menssage: 'Ingrese los parametros que se solicitan'})
    }
}
...
function listMatchUser(req, res){
    ...
    console.log(params.idLeague)
        ...
        Match.find({league: params.idLeague }).exec((err, matchFound) => {
            if (err) return res.status(500).send({ menssage: "error en la petición buscar match" })
            if (matchFound.length <= 0) return res.status(500).send({ menssage: 'No existe un match con ese id de liga' })
...
function listMatchAdmin(req, res) {
    let params = req.params;
        ...
        if (req.user.role == 0) {
            Match.find({league: params.idLeague}).exec((err, matchFound) => {
                if (err) return res.status(500).send({ menssage: 'Error en la peticion buscar liga' })
                if (matchFound.length <= 0) return res.status(500).send({ menssage: 'No existen partidos en esa liga' })
                return res.status(200).send({ matchFound })
            })
        } else {
            return res.status(500).send({ menssage: 'No posee permisos' })
